...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="xs fn">
			...
			<xsl:for-each select="configuration">
				...
				<hostgroup>
					<xsl:attribute name="sid">
   								 <xsl:value-of select="fn:string('-1')" />
 					</xsl:attribute>
					<xsl:attribute name="cid">
   								 <xsl:value-of select="./@cid" />
 					</xsl:attribute>
					<xsl:attribute name="type">
   								 	<xsl:value-of select="fn:string('hostgroup')" />
 					</xsl:attribute>
					<hostgroup_name>
				...
				<xsl:for-each select="$var1_hosts/host">
					...
					<xsl:for-each select="monitor_ip_address">
						...
						<host>
						 <xsl:attribute name="sid">
 						 ...
 						 <xsl:attribute name="cid">
   							 <xsl:value-of select="$var7_cur/@cid" />
 						 </xsl:attribute>
 						 <xsl:attribute name="type">
   								 	<xsl:value-of select="fn:string('host')" />
 						 </xsl:attribute>
							<host_name>
							...
							<display_name>
								<xsl:sequence select="fn:string($var7_cur/name)"/>
							</display_name>
							...
							<!-- Set the template according to the service type hosted -->
							<xsl:for-each select="$var3_applications/application">
								<xsl:variable name="cur_app" as="node()" select="."/>
								<xsl:for-each select="services/service">
									<xsl:variable name="cur_app_serv" as="node()" select="."/>
									<xsl:if test="$var7_cur/description = $cur_app_serv/run_on"> 
									<xsl:choose>
										<xsl:when test="string-length($cur_app_serv/ip_address)>0"> 
											<xsl:if test="$cur_app_serv/ip_address=fn:string($var6_resultof_cast)">
												<!--  <use_template>
														 <xsl:sequence select="fn:string($cur_app_serv/type)"/>
												</use_template> -->
												<hostgroups>
													<xsl:sequence select="fn:string($cur_app_serv/type)"/>
												</hostgroups>
							...
							<!-- End of set the template according to the service type hosted -->
							
							<use_variables>
								<hid>
									<xsl:sequence select="fn:string($var7_cur/@hid)"/>
								</hid>
								<xsl:if test="fn:exists($var7_cur/id) and fn:string($var7_cur/id)!=''">	
								
									<identifier>
										<xsl:sequence select="fn:string($var7_cur/id)"/>
									</identifier>
								</xsl:if>
								<xsl:if test="fn:exists($var7_cur/auth_user) and fn:string($var7_cur/auth_user)!=''">	
								...
								<xsl:if test="fn:exists($var7_cur/auth_pwd) and fn:string($var7_cur/auth_pwd)!=''">	
								<pwd>
									<xsl:sequence select="fn:string($var7_cur/auth_pwd)"/>
								</pwd>
								</xsl:if>
								<xsl:if test="fn:exists($var7_cur/domain_name) and fn:string($var7_cur/domain_name)!=''">	
								<domain>
					...
					<xsl:for-each select="monitor_ip_address">
						...
						<host>
							 <xsl:attribute name="sid">
 							 ...
 							 <xsl:attribute name="cid">
   								 <xsl:value-of select="$var9_cur/@cid" />
 						 	 </xsl:attribute>
 						 	 <xsl:attribute name="type">
   								 	<xsl:value-of select="fn:string('device')" />
 							 </xsl:attribute>
							<host_name>
	...
	<!--  							<xsl:choose>
				...
				<xsl:for-each select="$var3_applications/application">
					...
					<xsl:for-each select="services/service">
						...
						<servicegroup>
							...
							<xsl:attribute name="cid">
		   								 <xsl:value-of select="$var11_cur/@cid" />
		 					</xsl:attribute>
							<xsl:attribute name="type">
		   								 	<xsl:value-of select="fn:string('servicegroup')" />
		 					</xsl:attribute> 
							<alias>
								<xsl:sequence select="fn:string(description)"/>
							</alias>
							<servicegroup_name>
								<xsl:sequence select="fn:concat(fn:concat(fn:string(type), '@'),fn:concat(fn:concat(fn:string(id),'-'), fn:string($var11_cur/name)))"/>
							</servicegroup_name>
						<!-- 	<servicegroups_members>
							...
							<xsl:for-each select="$var1_hosts/host">
								...
								<xsl:for-each select="monitor_ip_address">
									...
									<xsl:if test="$var10_cur/description = $cur_serv/run_on"> 
									<xsl:choose>
										<xsl:when test="string-length($cur_serv/ip_address)>0"> 
											<xsl:if test="$cur_serv/ip_address=fn:string(.)">
												<xsl:variable name="hostgroup" select="fn:string($cur_serv/type)" />
												<xsl:variable name="hostname" select="fn:concat(fn:concat(fn:string($var10_cur/name), '@'), fn:string(.))"/>
												<members>
														<xsl:value-of select="fn:concat(fn:concat(fn:string($hostgroup), '::'), fn:string($hostname))"/> 
												</members>
												<notes>
														<xsl:value-of select="fn:concat(fn:concat(fn:string($hostgroup), '::'), fn:string($hostname))"/>
												</notes>
						...
						<!-- <service>
							 <xsl:attribute name="sid">
 							 ...
 							 <xsl:attribute name="type">
   								 	<xsl:value-of select="fn:string('service')" />
 						 	</xsl:attribute>
							...
							<check_command>
								<args>n=<xsl:sequence select="fn:string($cur_serv/process_name)"/></args>
							</check_command>
						</service>-->
 							 	...
 							 	<xsl:attribute name="type">
									...
									<xsl:for-each select="$var1_hosts/host">
										...
										<xsl:for-each select="monitor_ip_address">
											...
											<xsl:if test="$var22_cur/description = $cur_serv/run_on"> 
											<xsl:choose>
												<xsl:when test="string-length($cur_serv/ip_address)>0"> 
													<xsl:if test="$cur_serv/ip_address=fn:string(.)">
														<host_name>
									...
									<service_description>
										<xsl:sequence select="fn:concat(fn:concat(fn:string($cur_serv/type), ' '),fn:string(name))"/>
									</service_description>
									<check_command>
										<name><xsl:sequence select="fn:string(name)"/></name>
										<args>
										<xsl:choose>
				...
				<xsl:variable name="cur_app" as="node()" select="."/>
					<servicegroup>
						<xsl:attribute name="sid">
   								 <xsl:value-of select="$cur_app/@aid" />
 						</xsl:attribute>
					<xsl:attribute name="cid">
   								 <xsl:value-of select="./@cid" />
 					</xsl:attribute>
					<xsl:attribute name="type">
						...
						<xsl:for-each select="services/service">
							<xsl:variable name="cur_serv" as="node()" select="."/>
							<servicegroups_members>
									<!-- <xsl:sequence select="fn:concat(fn:concat(fn:string($cur_serv/type), '@'), fn:string($cur_app/id))"/> 
									<xsl:sequence select="fn:concat(fn:concat(fn:concat(fn:concat(fn:string($cur_serv/type),'-'),fn:string($cur_serv/id)), '@'), fn:string($cur_app/id))"/>-->
									<xsl:sequence select="fn:concat(fn:concat(fn:string($cur_serv/type), '@'),fn:concat(fn:concat(fn:string($cur_serv/id),'-'), fn:string($cur_app/name)))"/>
							</servicegroups_members>
						</xsl:for-each>
				  ...
				  <servicegroup>
					...
					<xsl:attribute name="type">
						...
						<servicegroup_name>
							<xsl:value-of select="fn:concat(fn:concat(fn:string(name), '@'), fn:string(identifier))" />
						</servicegroup_name>
					<!-- 	<servicegroup_description>
					...
					<xsl:for-each select="monitor_info/metrics/metric">
						...
						<service>
								<xsl:attribute name="sid">
 							 	...
 							 	<xsl:attribute name="type">
   								 	<xsl:value-of select="fn:string('metric')" />
 							 	</xsl:attribute>
 							 	<xsl:attribute name="cid">
   								 	<xsl:value-of select="$var38_cur/@cid" />
 						 	    </xsl:attribute>
							...
							<check_command>
								<name><xsl:sequence select="$var31_resultof_cast"/></name>
								<args>
								<xsl:choose>
					...
					<xsl:for-each select="monitor_info/metrics/metric">
						...
						<service>
								<xsl:attribute name="sid">
 							 	...
 							 	<xsl:attribute name="cid">
   								 	<xsl:value-of select="$var51_cur/@cid" />
 						 	    </xsl:attribute>
				...
				<xsl:for-each select="$var2_tenants/tenant">
					...
					<service>
						<xsl:attribute name="sid">
					 	...
					 	<xsl:attribute name="cid">
								 	<xsl:value-of select="$tenant_cur/@cid" />
				 	    </xsl:attribute>
						<host_name>
