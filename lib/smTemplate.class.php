...
class smTemplate 
{
	...
	function getTemplateVars($id)
	{
		...
		$tplVars=array();
		if(preg_match_all("/\{([a-zA-Z0-9\-_]+)\}/", $this->templates[$id],$matches))
		{
	...
	public function addTemplateData($id, $data) {
		...
		foreach($data as $key => $value) {
			...
			$value = (is_file($value)) ? $this->parseFile($value) : $value;
	...
	public function display($id) {
		...
		preg_match_all('~{?{([\w\d_]+?)}}?~', $result, $matches);
	 ...
	 */
	public function addCSS($filename, $template_id = 'main', $path = "") {
	 ...
	 * @param string $path uses default set in config if non specified
	 */
	public function addJS($filename, $template_id = 'main', $path = "") {
	 ...
	 * 
	 * @param string $id template id used in the tpl
	 * @param string $filename path to template file
	 */
	function removeJS($id,$filename)
	{
		unset ($this->js_files[$id][$filename]);
	}
	 ...
	 * @param string $filename path to template file
	 */
	function removeCSS($id,$filename)
	{
		unset ($this->css_files[$id][$filename]);
	}
