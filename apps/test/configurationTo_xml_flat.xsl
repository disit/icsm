				...
				<item>
					...
					<xsl:if test="string(boolean(string($var2_cid))) != 'false'">
						<xsl:attribute name="id">
							<xsl:value-of select="string(number(string($var1_cid)))"/>
						</xsl:attribute>
					</xsl:if>
					<content>
						<name>
					...
					<item>
						...
						<xsl:if test="string((string((string($var5_resultof_exists) != 'false')) != 'false')) != 'false'">
							<xsl:attribute name="parent_id">
								<xsl:value-of select="string(number(string($var3_cid)))"/>
							</xsl:attribute>
						</xsl:if>
							...
							<name>
								<xsl:if test="string((string((string($var5_resultof_exists) != 'false')) != 'false')) != 'false'">
									<xsl:attribute name="href">
										<xsl:value-of select="concat(concat(concat('configuration/view/', string(number(string($var3_cid)))), '/'), 'applications')"/>
									</xsl:attribute>
								</xsl:if>
			...
			<xsl:for-each select="configuration">
				...
				<xsl:for-each select="tenants">
					...
					<item>
						...
						<xsl:if test="string((string((string($var9_resultof_exists) != 'false')) != 'false')) != 'false'">
							<xsl:attribute name="parent_id">
								<xsl:value-of select="string(number(string($var7_cid)))"/>
							</xsl:attribute>
						</xsl:if>
						...
						<content>
							<name>
								<xsl:if test="string((string((string($var9_resultof_exists) != 'false')) != 'false')) != 'false'">
									<xsl:attribute name="href">
										<xsl:value-of select="concat(concat(concat('configuration/view/', string(number(string($var7_cid)))), '/'), 'tenants')"/>
									</xsl:attribute>
								</xsl:if>
			...
			<xsl:for-each select="configuration">
				...
				<xsl:for-each select="hosts">
					...
					<item>
						...
						<xsl:if test="string((string((string($var13_resultof_exists) != 'false')) != 'false')) != 'false'">
							<xsl:attribute name="parent_id">
								<xsl:value-of select="string(number(string($var11_cid)))"/>
							</xsl:attribute>
						</xsl:if>
						...
						<content>
							<name>
								<xsl:if test="string((string((string($var13_resultof_exists) != 'false')) != 'false')) != 'false'">
									<xsl:attribute name="href">
										<xsl:value-of select="concat(concat(concat('configuration/view/', string(number(string($var11_cid)))), '/'), 'hosts')"/>
									</xsl:attribute>
								</xsl:if>
			...
			<xsl:for-each select="configuration">
				...
				<xsl:for-each select="devices">
					...
					<item>
						...
						<xsl:if test="string((string((string($var17_resultof_exists) != 'false')) != 'false')) != 'false'">
							<xsl:attribute name="parent_id">
						...
						<content>
							<name>
								<xsl:if test="string((string((string($var17_resultof_exists) != 'false')) != 'false')) != 'false'">
									<xsl:attribute name="href">
										<xsl:value-of select="concat(concat(concat('configuration/view/', string(number(string($var15_cid)))), '/'), 'devices')"/>
									</xsl:attribute>
								</xsl:if>
							...
							<name>
								...
								<xsl:if test="string(boolean(string($var20_cid))) != 'false'">
									...
									<xsl:if test="string(boolean(string($var22_aid))) != 'false'">
										<xsl:attribute name="href">
											<xsl:value-of select="concat(concat(concat(concat(concat(concat('configuration/view/', string(number(string($var19_cid)))), '/'), 'applications'), '/'), 'aid:'), string(number(string($var21_aid))))"/>
										</xsl:attribute>
									</xsl:if>
								</xsl:if>
							...
							<name>
								...
								<xsl:if test="string(boolean(string($var25_cid))) != 'false'">
									...
									<xsl:if test="string(boolean(string($var27_tid))) != 'false'">
										<xsl:attribute name="href">
											<xsl:value-of select="concat(concat(concat(concat(concat(concat('configuration/view/', string(number(string($var24_cid)))), '/'), 'tenants'), '/'), 'tid:'), string(number(string($var26_tid))))"/>
										</xsl:attribute>
									</xsl:if>
								</xsl:if>
								<xsl:value-of select="string(name)"/>
							...
							<name>
								...
								<xsl:if test="string(boolean(string($var30_cid))) != 'false'">
									...
									<xsl:if test="string(boolean(string($var32_hid))) != 'false'">
										<xsl:attribute name="href">
											<xsl:value-of select="concat(concat(concat(concat(concat(concat('configuration/view/', string(number(string($var29_cid)))), '/'), 'hosts'), '/'), 'hid:'), string(number(string($var31_hid))))"/>
										</xsl:attribute>
									</xsl:if>
								</xsl:if>
								<xsl:value-of select="string(name)"/>
							...
							<name>
								...
								<xsl:if test="string(boolean(string($var35_cid))) != 'false'">
									...
									<xsl:if test="string(boolean(string($var37_did))) != 'false'">
										<xsl:attribute name="href">
											<xsl:value-of select="concat(concat(concat(concat(concat(concat('configuration/view/', string(number(string($var34_cid)))), '/'), 'devices'), '/'), 'did:'), string(number(string($var36_did))))"/>
										</xsl:attribute>
									</xsl:if>
								</xsl:if>
								<xsl:value-of select="string(name)"/>
